...
import { Globe, Smartphone, Database, Sparkles, Plus, Trash2, ArrowLeft, ArrowRight, Loader2, ShieldCheck, BadgeCheck, Lightbulb, X } from "lucide-react";
...
function PostJob() {
  ...
  const addMilestone = () => setData({ ...data, milestones: [...data.milestones, { title: "", amount: "", due: "" }] });
  const updateMilestone = (i: number, k: keyof Milestone, v: string) => {
    const next = [...data.milestones];
    next[i] = { ...next[i], [k]: v };
    setData({ ...data, milestones: next });
  };
  const removeMilestone = (i: number) => setData({ ...data, milestones: data.milestones.filter((_, x) => x !== i) });
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-2 text-sm">
        <div className="font-medium">Step {step} of 4: <span className="text-primary">{stepLabels[step - 1]}</span></div>
      ...
      <div className="grid lg:grid-cols-[1fr,320px] gap-6">
        <div className="rounded-2xl border border-border/60 bg-card p-6 md:p-8">
          {step === 1 && (
            <>
              ...
              <p className="text-sm text-muted-foreground mt-2">Describe your project requirements clearly to attract the most qualified developers across Africa.</p>
              <div className="mt-6 space-y-5">
                <div>
                  <Label className="flex items-center gap-1.5">Job Title <BadgeCheck className="h-3.5 w-3.5 text-primary" /></Label>
                  <Input className="mt-1.5" placeholder="e.g., Senior Full Stack Engineer for Fintech Dashboard" value={data.title} onChange={(e) => setData({ ...data, title: e.target.value })} />
                  <div className="text-xs text-muted-foreground mt-1.5">Include key technologies and seniority level to improve search visibility.</div>
                </div>
                <div>
                  ...
                  <div className="grid sm:grid-cols-3 gap-3 mt-1.5">
                    {categories.map((c) => {
                      ...
                      return (
                        <button key={c.id} type="button" onClick={() => setData({ ...data, category: c.id })}
                ...
                <div>
                  ...
                  <Textarea rows={6} className="mt-1.5" placeholder="Describe the scope, technical requirements, and deliverable expectations…" value={data.description} onChange={(e) => setData({ ...data, description: e.target.value })} />
                </div>
              </div>
            </>
          )}
          ...
          {step === 2 && (
            <>
              ...
              <p className="text-sm text-muted-foreground mt-2">Search and select the technical stack for your project. Be specific to attract the right talent.</p>
              <div className="mt-6">
                <Label>Search Skills</Label>
                <Input className="mt-1.5" placeholder="e.g. React, Python, Solidity… (press Enter)"
                  value={data.skillInput}
                  onChange={(e) => setData({ ...data, skillInput: e.target.value })}
                  onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addSkill(data.skillInput); } }} />
                <div className="flex flex-wrap gap-2 mt-3">
                  {data.skills.map((s) => (
                    <span key={s} className="inline-flex items-center gap-1.5 rounded-full bg-primary/15 text-primary px-3 py-1 text-sm">
                ...
                {(suggestedSkills[data.category] ?? []).length > 0 && (
                  <div className="mt-4 text-xs text-muted-foreground">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1.5">
                      {suggestedSkills[data.category].filter((s) => !data.skills.includes(s)).map((s) => (
                        <button key={s} onClick={() => addSkill(s)} className="rounded-full border border-border/60 px-2.5 py-1 text-xs hover:border-primary/40 hover:text-primary">+ {s}</button>
                      ))}
                    </div>
                  </div>
                )}
          ...
          {step === 3 && (
            <>
              ...
              <p className="text-sm text-muted-foreground mt-2">Define the total scope. Break the work into manageable payments released only when milestones are approved.</p>

              <div className="mt-6 rounded-xl border border-border/60 bg-surface/40 p-5">
                <Label>Total Project Budget (USD)</Label>
                <Input type="number" min="0" className="mt-1.5 text-lg" placeholder="4500.00" value={data.budget} onChange={(e) => setData({ ...data, budget: e.target.value })} />
                <Input className="mt-3" placeholder="Estimated duration (e.g. 4–6 weeks)" value={data.duration} onChange={(e) => setData({ ...data, duration: e.target.value })} />
              </div>

              <div className="mt-6">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-display font-semibold">Milestone Breakdown</h3>
                  <Button type="button" variant="ghost" size="sm" onClick={addMilestone} className="text-primary"><Plus className="h-4 w-4 mr-1" /> Add Milestone</Button>
                </div>
                <div className="space-y-3">
                  {data.milestones.map((m, i) => (
                    <div key={i} className="rounded-xl border border-border/60 p-4 grid grid-cols-1 md:grid-cols-[1.5fr,1fr,1fr,auto] gap-3 items-end">
                      <div><Label className="text-xs">Milestone {i + 1}</Label><Input className="mt-1" placeholder="UI/UX Design & Prototype" value={m.title} onChange={(e) => updateMilestone(i, "title", e.target.value)} /></div>
                      <div><Label className="text-xs">Amount ($)</Label><Input className="mt-1" type="number" min="0" placeholder="1500" value={m.amount} onChange={(e) => updateMilestone(i, "amount", e.target.value)} /></div>
                      <div><Label className="text-xs">Estimated Date</Label><Input className="mt-1" type="date" value={m.due} onChange={(e) => updateMilestone(i, "due", e.target.value)} /></div>
                      <button onClick={() => removeMilestone(i)} className="h-9 w-9 rounded-md border border-border/60 hover:border-destructive/60 hover:text-destructive flex items-center justify-center"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {step === 4 && (
            <>
              ...
              <p className="text-sm text-muted-foreground mt-2">Make sure everything looks good before funding the escrow.</p>
              <div className="mt-6 rounded-xl border border-border/60 p-5">
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Project Title</div>
              ...
              <div className="mt-4 rounded-xl border border-primary/30 bg-primary/5 p-5 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Funds are held in a multi-signature wallet. Payment is released only when you approve each milestone. Mediation team available 24/7.</p>
                </div>
              </div>
            </>
          )}
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-border/40">
            <Button variant="ghost" disabled={step === 1} onClick={() => setStep(step - 1)}><ArrowLeft className="h-4 w-4 mr-2" /> Back</Button>
            {step < 4 ? (
              <Button disabled={!canNext} onClick={() => setStep(step + 1)} className="bg-[image:var(--gradient-primary)] text-primary-foreground">
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-accent/30 bg-accent/10 p-5">
            <div className="flex items-center gap-2 text-accent"><Lightbulb className="h-4 w-4" /> <div className="font-display font-semibold">Expert Tip</div></div>
            <p className="text-sm mt-2 text-foreground/90">Projects with a detailed technical breakdown receive 40% more high-quality proposals in the first 24 hours.</p>
          </div>
          {step === 4 && (
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-5">
            <div className="font-display font-semibold flex items-center gap-2"><Sparkles className="h-4 w-4 text-primary" /> Verified Marketplace</div>
            <p className="text-xs text-muted-foreground mt-2">Top 5% African tech talent. All developers undergo technical vetting before joining.</p>
          </div>
        </aside>
      </div>
    </div>
